...
console.log(sc)
    if (sc == 'S') {
        if (modules[message.guild.id].global[target]) {
            modules[message.guild.id].global[target] = false
            message.reply('O módulo ' + target + ' foi desabilitado globalmente!')
        } else {
            modules[message.guild.id].global[target] = true
            message.reply('O módulo ' + target + ' foi habilitado globalmente!')
        }
    } else {
        if (modules[message.guild.id].channels[message.channel.id][target]) {
